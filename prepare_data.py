...
def prepare_data(data_dir, output_dir, pipeline="cpac", quality_checked=True):
    ...
    try:  # check if feature file already exists
        # load features
        feat_file = os.path.join(output_dir, "ABIDE_BASC064_features_1D.npz")
        ...
        # Save features
        np.savez_compressed(
            os.path.join(output_dir, "ABIDE_BASC064_features_1D"), a=X_features
        )
    ...
    return (X_features, y_target, abide_pheno)
...
def load_data(data_dir, output_dir):
    ...
    phenotypic_file = os.path.join(data_dir, "ABIDE_pcp/abide_pheno.csv")
    abide_pheno = pd.read_csv(phenotypic_file)
    y_target = abide_pheno["DX_GROUP"]
